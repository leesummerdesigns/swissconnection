...
export default async function PublicProfilePage({
  ...
  const translatedDays = [
    { key: "mon", label: ta("monday") },
    { key: "tue", label: ta("tuesday") },
    { key: "wed", label: ta("wednesday") },
    { key: "thu", label: ta("thursday") },
    { key: "fri", label: ta("friday") },
    { key: "sat", label: ta("saturday") },
    { key: "sun", label: ta("sunday") },
  ];
  const translatedSlots = [
    { key: "morning", label: ta("morning") },
    { key: "afternoon", label: ta("afternoon") },
    { key: "evening", label: ta("evening") },
  ];
  const translatedDayHeader = ta("day");
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-sm text-text-secondary">
                ...
                <span className="flex items-center gap-1">
                  <Clock size={14} />
                  {t("memberSince", {
                    date: new Date(user.createdAt).toLocaleDateString(params.locale, {
                      month: "long",
                      year: "numeric",
                    }),
                  })}
          ...
          {isProvider && Object.keys(availability).length > 0 && (
            <section className="mb-8">
              <h2 className="text-lg font-semibold mb-4">{t("availability")}</h2>
              <AvailabilityDisplay
                availability={availability}
                days={translatedDays}
                slots={translatedSlots}
                dayHeader={translatedDayHeader}
              />
            </section>
          )}
          ...
          {user.recommendationsReceived.length > 0 && (
            <section className="mb-8">
              <h2 className="text-lg font-semibold mb-4">
                <span className="flex items-center gap-2">
                  <Award size={18} />
                  {t("recommendationsCount", { count: user.recommendationsReceived.length })}
                </span>
              </h2>
                ...
                {translatedRecommendations.map((rec) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-text-tertiary">
                      ...
                      {new Date(rec.createdAt).toLocaleDateString(params.locale, {
                        month: "long",
                        year: "numeric",
                      })}
                    </p>
          ...
          <section className="mb-8">
            {user.reviewsReceived.length > 0 && (
              <>
                <h2 className="text-lg font-semibold mb-4">
                  {t("reviewsCount", { count: user.reviewsReceived.length })}
                </h2>
          ...
          <div className="sticky top-24 space-y-4">
            ...
            <div className="card p-6">
              <h3 className="font-semibold text-lg mb-4">
                {t("contact", { name: user.name.split(" ")[0] })}
              </h3>
              <Link
            ...
            <div className="card p-6">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-text-secondary">{t("reviewsReceived")}</span>
                  <span className="font-medium">
                    {user.reviewsReceived.length}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-secondary">{t("reviewsWritten")}</span>
                  <span className="font-medium">
                    {user.reviewsWritten.length}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-secondary">{t("recommendations")}</span>
                  <span className="font-medium">
                ...
                {avgRating > 0 && (
                  <div className="flex justify-between">
                    <span className="text-text-secondary">{t("avgRating")}</span>
